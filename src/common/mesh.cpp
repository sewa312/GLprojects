...
void Mesh::Draw(bool useNormals, bool useTexture)
{
	...
	auto texture = useTexture ? std::make_unique<UseVertexAttribPointer>(
		VertexAttr::TEXCOORD0, 3, GL_FLOAT, false, 0, &Coords[0]) : nullptr;
...
void CreateReelMesh(Mesh &mesh, int faces)
{
	...
	for (int i = 0; i < faces; i++)
	{
		...
		mesh.Coords.push_back(glm::vec2(0.05f, i * 1.0f / (faces - 1)));
		mesh.Coords.push_back(glm::vec2(0.95f, i * 1.0f / (faces - 1)));
